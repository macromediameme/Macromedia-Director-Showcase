...
EXTERN_BEGIN_DEFINE_CLASS_INSTANCE_VARS(CSrvXtn)
	...
	MovieLogDataList *	mMovieDataList;		// List of data for the movies

	bool				mLoggingEnabled;	// On or off flag
	long				mLastLogDay;		// Server day value
	long				mIdleCounter;		// Counter for skipping time checks
	BString				mLogFilePath;		// Folder where log files go

	BString				mServerStartupTime;	// Time the server started (xtra was loaded)
	BString				mLogStartTime;		// Time we've started the log data



	#define		kNumPermissions		5
	long				mPermissions[kNumPermissions];	// User levels required to execute commands
