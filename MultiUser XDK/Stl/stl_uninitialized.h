...
#  define __array_uninitialized_copy uninitialized_copy
#  define __array_copy copy
#  define __array_copy_backward copy_backward
# else

template <class T1, class T2>
inline T2*
__array_uninitialized_copy_aux(const T1* first, const T1* last,T2* result,
                               __true_type) {
  return __array_copy(first, last, result);
}
...
template <class T1, class T2>
inline T2*
  __array_uninitialized_copy(const T1* first, const T1* last, T2* result) {
  typedef typename __type_traits<T2>::is_POD_type is_POD_type;
  return __array_uninitialized_copy_aux(first, last, result, 
                                        is_POD_type());
}
...
inline pair<BidirectionalIterator, ForwardIterator>
