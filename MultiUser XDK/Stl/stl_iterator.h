...
//   bring up portions of Plum Hall Library with a wrapper
#     include <ph_iterator.h> 
#   endif
#   if defined (__MWERKS__ ) && (__MWERKS__ >= 0x1800) 
//    bring up portions of Modena Standard Library with a wrapper
#     include <msl_iterator.h>
#   endif /* __MWERKS__ */
#  else
...
// hierarchy causes ambiguity for some compilers having this bug
...
template <class Category, class T, __DFL_TMPL_PARAM(Distance,ptrdiff_t),
          __DFL_TMPL_PARAM(Pointer,T*), __DFL_TMPL_PARAM(Reference,T&) >
struct iterator {
...
// fbp : that order keeps gcc happy
template <class T>
...
INLINE_LOOP void __distance(InputIterator first, InputIterator last, 
...
INLINE_LOOP void __distance(ForwardIterator first, ForwardIterator last,
                       Distance& n, 
...
# if defined (__STL_NONTEMPL_BASE_MATCH_BUG) /* OBSOLETE */
template <class ForwardIterator, class Distance>
...
# define __Reference Reference, class Pointer
# define Reference__ Reference, Pointer
template <class BidirectionalIterator, class T, 
    __DFL_TMPL_PARAM(Reference, T& ),
    __DFL_TMPL_PARAM(Pointer, T*),
    __DFL_TYPE_PARAM(Distance, ptrdiff_t)> 
# else
# define __Reference Reference
# define Reference__ Reference
...
//  Use of operator->() without Pointer param may be inconsistent
    pointer operator->() const { return (pointer)&(operator*()); }
#endif /* __STL_ARROW_OPERATOR */
    self& operator++() {
        --current;
        return *this;
    }
...
template <class BidirectionalIterator, class T, class __Reference, 
          class Distance>
inline bidirectional_iterator_tag
iterator_category(const reverse_bidirectional_iterator<BidirectionalIterator, T,
                  Reference__, Distance>&) {
    return bidirectional_iterator_tag();
}
...
inline bool operator==(
    const reverse_bidirectional_iterator<BidirectionalIterator, T, Reference__,
                                         Distance>& x, 
    const reverse_bidirectional_iterator<BidirectionalIterator, T, Reference__,
                                         Distance>& y) {
...
template <class RandomAccessIterator, class T, __DFL_TMPL_PARAM(Reference,T&),
# if defined (__STL_MSVC50_COMPATIBILITY)
        __DFL_TMPL_PARAM(Pointer, T*),
# endif
        __DFL_TYPE_PARAM(Distance,ptrdiff_t)> 
class reverse_iterator {
    typedef reverse_iterator<RandomAccessIterator, T, Reference__, Distance>
        self;
# if ! defined (__STL_NO_FORWARD_FRIENDS)
    friend inline bool operator==(const self& x, const self& y);
    friend inline bool operator<(const self& x, const self& y);
    friend inline Distance operator-(const self& x, const self& y);
    friend inline self operator+(Distance n, const self& x);
# endif
protected:
...
# endif
    ...
    reverse_iterator() {}
    explicit reverse_iterator(const RandomAccessIterator& x) : current(x) {}
...
template <class RandomAccessIterator, class T, class __Reference, class Distance>
inline random_access_iterator_tag
...
inline bool operator==(const reverse_iterator<RandomAccessIterator, T,
                                              Reference__, Distance>& x, 
                       const reverse_iterator<RandomAccessIterator, T,
                                              Reference__, Distance>& y) {
    return x.base() == y.base();
}
...
template <class RandomAccessIterator, class T, class __Reference, class Distance>
inline reverse_iterator<RandomAccessIterator, T, Reference__, Distance> 
operator+(Distance n,
          const reverse_iterator<RandomAccessIterator, T, Reference__,
                                 Distance>& x) {
    return reverse_iterator<RandomAccessIterator, T, Reference__, Distance>
        (x.current - n);
}
